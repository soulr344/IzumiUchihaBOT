        ...
        for id in userIds:
            try:
                ...
                try:
                    ...
                    LOGGER.exception("ERROR banning user %s in chat %s (%s) due to %s", integerID, chat.title, chat.id,
                                     excp.message)
                    message.reply_text("Well damn, I can't ban " + str(integerID))
            except ValueError:
                message.reply_text("Error parsing the ID: " + id + " is not a valid user ID")
            except BadRequest as excp:
...
def temp_ban(update: Update, context: CallbackContext):
    ...
    try:
        ...
        reply = "\nReason: <code>{}</code>".format(reason) if reason else ""
        message.reply_text("Banned! {} will be banned for {}.".format(mention_html(member.user.id, member.user.first_name), time_val) + reply, parse_mode=ParseMode.HTML)
        return log
...
RIPME_HANDLER = DisableAbleCommandHandler("rip", kickme, filters=Filters.chat_type.groups, run_async=True)
AFK_HANDLER = DisableAbleCommandHandler("afk", kickme, filters=Filters.chat_type.groups, run_async=True)
